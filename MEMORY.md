...
- `tests/e2e/run.js` — headed Chrome (Puppeteer), resets DB at start + end. Runs `mis`, `roles`, and/or `admin` suites (`node tests/e2e/run.js [all|mis|roles|admin]`). Current total: 207 checks, all passing.
...
- (this session, WORKING TREE — next commit) **role-specific dashboards + panchayat/village demo users**: `RoleDashboardService` (`roleOf`/`stats` with unit/sub-units/users/records/forms), `User::homeUrl()` routing (login + sidebar + `dashboard.php` guard), 5 thin `mis/home_*.php` pages + shared `common/views/partials/role_dashboard.php`, idempotent demo-user seed (adds `pm_panchayat`, `vp_village`), portal seed updated; smoke section 16; E2E new `roles` suite (+21 checks → 203), all green.
