...
- `tests/e2e/run.js` — headed Chrome (Puppeteer), resets DB at start + end. Runs `mis`, `roles`, and/or `admin` suites (`node tests/e2e/run.js [all|mis|roles|admin]`). Current total: 203 checks, all passing.
...
- **Core services:** `common/src/Services/SurveyService.php`, `common/src/Services/RecordService.php` (static `scopeUserIds`/`canView`, `find()`, `submitted_by`), `common/src/Services/ReportService.php` (viewer-scoped), `common/src/Services/RoleDashboardService.php` (`roleOf`/`stats` for role landing pages), `common/src/Services/UserService.php` (scope + access)
