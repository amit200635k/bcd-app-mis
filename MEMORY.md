...
- `tests/e2e/run.js` — headed Chrome (Puppeteer), resets DB at start + end. Runs `mis` and/or `admin` suites (`node tests/e2e/run.js [all|mis|admin]`). Current total: 182 checks, all passing.
...
- (this session, WORKING TREE — next commit) **detailed, filterable records report + all-column filters + scope-aware KPIs**: new `mis/detail_report.php`, `ReportService` `DETAIL_COLUMNS`/`DETAIL_FILTERS`/`detailColumns`/`detailWhere` (generic EXISTS per column; district/block via location JSON; numeric range with REGEXP guard)/`detailReport`/`detailKpis` (adds Departments + Categories)/`detailFieldDistinct`/`detailLocationDistinct`/`detailSurveyors`; sidebar "Detailed Report" link; smoke section 15b; E2E +9 checks → 182, all green (KPI labels are `text-uppercase`, so E2E compares lower-cased text); CSV export URL excludes `viewer`.
...
- **MIS pages:** `mis/monitoring.php` (scoped + View button), `mis/records.php` (record detail — answers/images/GPS/workflow), `mis/masters/index.php` (all masters + inline items), `mis/reports.php` (scoped + CSV), `mis/detail_report.php` (detailed report: all-column filters + scope-aware KPIs + CSV), `mis/users/index.php` (portal + form access modal), `mis/users/save.php` (actor-scoped), `mis/login.php` (portal gate)
